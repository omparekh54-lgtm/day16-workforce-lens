...
import { parseCSV, normalizeRows, summarize, forecastCoverage, transferPlan, exportActions, generateDemoCSV } from '../lib/workforce.mjs';
...
test('demo exercises cross-team reallocation workflow',()=>{const rows=normalizeRows(parseCSV(generateDemoCSV())).rows;const f=forecastCoverage(rows,{horizon:14,targetUtilization:.85,serviceQuantile:.9});const p=transferPlan(f);assert.ok(p.some(a=>a.kind==='REALLOCATE'));assert.ok(p.some(a=>a.kind==='ADD_CAPACITY'));});
