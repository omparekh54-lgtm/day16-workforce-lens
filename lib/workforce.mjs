...
export function generateDemoCSV(){
  ...
  const lines=[header];
  const teams=[['Fulfillment',3,12],['Support',3,8],['Returns',3,9.5]];
  ...
  for(let d=0;d<42;d++){
    ...
    for(const [team,n,rate] of teams){
      ...
      for(let i=0;i<n;i++){
        ...
        const utilizationBase=team==='Returns' ? (0.48+((d+i)%5)*.015) : (0.82+((d+i)%5)*.025);
        const productive=Math.max(0,8-absence)*utilizationBase;
